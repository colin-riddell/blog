...
title = "Sharing dependencies between Spring Microservices with Maven"
description = ""
slug = ""
...
Sharing dependencies between Spring Microservices with Maven for better code-reuse and clean design

-------

## What's the problem

You're looking at microservices, but you realise you want to send an object from on service to the other. You write a class, and then you copy and paste it from the code of one service to the other ... 💡 there must be a better way to do this. You're a good engineer and you know that copying around this POJO is bad practice. Well there is a better way - but if you're using Maven and Spring (Boot) it can be a little tricky. Read on to see how to solve that.


## Setting up a parent POM	

We want to be able to create a common project for sharing dependencies between different Java based microservices. To do that we need to:

* Create a "parent" maven - this is a `pom.xml` which will sit outside all of the projects that make up our microservices and allow us to tie them together
...
Notice that the `packaging` property is set to `pom` - aggregator projects (parent projects) need to be set as POM packaging.
...
Find this code and remove it, including the `parent` tags - versions may differ. Open the 
...
We want to set the parent to the project we setup earlier. To do this, we will set the parent to the parents artifact name `ordering-services` and of course match the `groupId`. `version` needs to match, too.
...
We will create a controller in `sales-service` that will use `Monkey` to demonstrate this point.
...
If we want to make sure that Jar files are generated for every project in our tree, then the maven property `packaging` should be set to jar for each project we want a jar from when we ask for all of the projects to be packaged. Add this just below the project version.
...
We also need to enable the `repackaging` goal on the `spring-boot-maven-plugin`. This needs to be done if we're going to run the build jars from the command line with `java -jar`.
